...
static void exampleWill() {
	...
	while (!WindowShouldClose()) {
		BeginDrawing();
		DrawTexture(texture->getResizedExWill(), GetScreenWidth() / 2 - 360, GetScreenHeight() / 2 - 350, WHITE);
...
static void digWillDef() {
	...
	const Rectangle makeAnExampleWill = { ((float)GetScreenWidth() / 2) - 125, ((float)GetScreenHeight() / 2) + 180, 270, 100 };
	const Font customFont = LoadFont("../assets/fonts/roboto.ttf");
